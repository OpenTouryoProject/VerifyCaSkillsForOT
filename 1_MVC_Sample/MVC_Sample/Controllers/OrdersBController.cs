...
namespace MVC_Sample.Controllers
{
    ...
    /// </summary>
    [Authorize]
    public class OrdersBController : MyBaseMVController
    {
        /// <summary>編集中の DataTable を置く Session のキー</summary>
        /// <remarks>★ net48 は Session に DataTable をそのまま置ける（binary 直列化）。</remarks>
        ...
        {
            return this.Session[OrdersBController.SessionKey] as DataTable;
        }

        /// <summary>編集中の DataTable を Session に保持する（null で破棄）</summary>
        private void SaveEditingTable(DataTable dt)
        {
            if (dt == null) { this.Session.Remove(OrdersBController.SessionKey); }
            else { this.Session[OrdersBController.SessionKey] = dt; }
        }
        ...
        [HttpGet]
        public async Task<ActionResult> Index(OrdersViewModel model)
        {
        ...
        public async Task<ActionResult> Search(OrdersViewModel model)
        {
            model.PageIndex = 1;
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> Page(OrdersViewModel model, int targetPage)
        {
        ...
        /// <returns>再描画（ViewResult）</returns>
        private async Task<ActionResult> SearchPage(OrdersViewModel model, string message)
        {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> AddRow(OrdersViewModel model)
        {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> UpdateRow(OrdersViewModel model, int rowIndex)
        {
            DataTable dt = this.LoadEditingTable();
            if (dt == null) { return await this.NoListYet(model); }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> DeleteRow(OrdersViewModel model, int rowIndex)
        {
            DataTable dt = this.LoadEditingTable();
            if (dt == null) { return await this.NoListYet(model); }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> BatchUpdate(OrdersViewModel model)
        {
            DataTable dt = this.LoadEditingTable();
            if (dt == null) { return await this.NoListYet(model); }
        ...
        private async Task<ActionResult> NoListYet(OrdersViewModel model)
        {
        ...
        private async Task<ActionResult> Rebind(OrdersViewModel model, DataTable dt, string message)
        {
            model.Orders = dt;
