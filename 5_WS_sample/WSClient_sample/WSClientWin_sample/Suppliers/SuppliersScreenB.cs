...
//* クラス日本語名  ：Suppliers 画面Ｂ（一覧＆更新：RowState バッチ）／3層WSクライアント
//*
//* 作成日時        ：2026/08/27
//* 作成者          ：生技
//* 更新履歴        ：
...
namespace WSClientWin_sample.Suppliers
{
    ...
    /// ［バッチ更新］で 通信制御機能 → Ｂ層 → 自動生成Dao 経由で一括反映する。
    ...
    public class SuppliersScreenB : SuppliersBaseForm
    {
        ...
        {
            this.Text = "Suppliers 画面Ｂ（一覧＆バッチ更新）／3層";
            this.Width = 1020;
            this.Height = 560;
            ...
            this.labelMessage.Size = new Size(860, 20);
            ...
            this.dgvSuppliers.Size = new Size(972, 400);
        ...
        {
            SuppliersReturnValue returnValue = this.CallService("SuppliersSelectAll", null, rcFxEventArgs.ControlName);
        ...
        protected void UOC_btnMain2_Click(RcFxEventArgs rcFxEventArgs)
        {
            ...
            SuppliersReturnValue returnValue = this.CallService("SuppliersBatchUpdate", this.dtSuppliers, rcFxEventArgs.ControlName);
            if (returnValue == null) { return; }
            ...
            SuppliersReturnValue reloadRv = this.CallService("SuppliersSelectAll", null, rcFxEventArgs.ControlName);
            if (reloadRv != null)
            {
        ...
        /// <summary>通信制御機能経由でＢ層を呼び出す</summary>
        ...
        /// <remarks>
        /// ★ 3層は CallController.Invoke（サービス論理名）＝サーバ側がコミットする。
        ///   2CS のような手動 Commit/Rollback は不要（呼んではいけない）。
        /// </remarks>
        private SuppliersReturnValue CallService(string methodName, DataTable suppliers, string controlName)
        {
            // ↓Ｂ層実行（通信制御機能経由）-------------------------------------
            ...
            {
                // 業務例外＝サーバ側でロールバック済み。RowState は残るのでやり直せる。
