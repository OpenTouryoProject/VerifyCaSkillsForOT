...
namespace WSClientWin_sample.Orders
{
    ...
    public class OrdersScreenA : OrdersBaseForm
    {
        /// <summary>結果表示ラベル</summary>
        private Label labelMessage;

        /// <summary>コンストラクタ</summary>
        public OrdersScreenA()
        {
            this.Text = "Orders 画面Ａ（件数確認）";
            this.Width = 720;
            this.Height = 260;
            this.StartPosition = FormStartPosition.CenterScreen;

            Label guide = new Label();
            guide.Text = "［件数確認］で Orders のデータ件数を共通Dao 経由で取得し、メッセージ ダイアログで表示します。"
                + "\r\n［一覧へ］で画面Ｂ（条件検索・ページング・バッチ更新）を開きます。";
        ...
        {
            // ↓Ｂ層実行：Orders の件数確認------------------------------------
            // ★ 3層は URL でもクラス名でもなく「サービス論理名」で呼ぶ。
            //   定義ファイルが実体を解決し、インプロセス⇄WCF/WebAPI をコード無変更で切替。
            //   分離レベルはサーバ側が決めるので iso は渡せない（コミットもサーバ側）。
            OrdersParameterValue parameterValue = new OrdersParameterValue(
                this.Name, rcFxEventArgs.ControlName, "OrdersSelectCount", "SQL",
                MyBaseControllerWin.UserInfo);

            CallController callController = new CallController(MyBaseControllerWin.UserInfo);
            OrdersReturnValue returnValue =
                (OrdersReturnValue)callController.Invoke(this.GetServiceName(), parameterValue);
        ...
        protected void UOC_btnMain2_Click(RcFxEventArgs rcFxEventArgs)
        {
            ...
            using (OrdersScreenB dialog = new OrdersScreenB())
            {
