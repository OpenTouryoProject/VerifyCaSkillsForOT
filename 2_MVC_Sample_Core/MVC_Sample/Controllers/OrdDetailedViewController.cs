...
namespace MVC_Sample.Controllers
{
    ...
    public class OrdDetailedViewController : MyBaseMVControllerCore
    {
        ...
        /// <summary>編集中の明細の DataTable のテーブル名</summary>
        private const string DetailTableName = "OrderDetails";
        ...
        private DataTable LoadEditingRow()
        {
            return this.LoadFromSession(
                OrdDetailedViewController.SessionKey, OrdDetailedViewController.TableName);
        }

        /// <summary>編集中の1行を Session に保持する（null で破棄）</summary>
        private void SaveEditingRow(DataTable dt)
        {
            this.SaveToSession(OrdDetailedViewController.SessionKey, dt);
        }
        ...
        {
            return this.LoadFromSession(
                OrdDetailedViewController.SessionKeyDetails, OrdDetailedViewController.DetailTableName);
        }

        /// <summary>編集中の明細を Session に保持する（null で破棄）</summary>
        private void SaveEditingDetails(DataTable dt)
        {
            this.SaveToSession(OrdDetailedViewController.SessionKeyDetails, dt);
        }

        /// <summary>Session（JSON）から DataTable を復元する</summary>
        /// <param name="key">Session のキー</param>
        /// <param name="tableName">テーブル名</param>
        /// <returns>DataTable（無ければ null）</returns>
        private DataTable LoadFromSession(string key, string tableName)
        {
            string json = this.HttpContext.Session.GetString(key);
            if (string.IsNullOrEmpty(json)) { return null; }

            return DTTables.JsonToDTTables(json).ToDataSet().Tables[tableName];
        }

        /// <summary>DataTable を Session（JSON）に保持する（null で破棄）</summary>
        /// <param name="key">Session のキー</param>
        /// <param name="dt">DataTable</param>
        private void SaveToSession(string key, DataTable dt)
        {
            if (dt == null)
            {
            ...
            this.HttpContext.Session.SetString(key, DTTables.DTTablesToJson(dtts));
        }
        ...
        public async Task<IActionResult> AddDetailRow(OrdViewModel model)
        {
            await this.LoadMasters(model);
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> UpdateDetailRow(OrdViewModel model, int rowIndex)
        {
            await this.LoadMasters(model);
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> DeleteDetailRow(OrdViewModel model, int rowIndex)
        {
            await this.LoadMasters(model);
        ...
        /// <param name="model">OrdViewModel</param>
        /// <returns>再描画（ViewResult）</returns>
        private IActionResult NoTarget(OrdViewModel model)
        {
